...
void UGunInput::FireShot()
{
	...
	if (GetWorld()->LineTraceSingleByChannel(Hit, StartTrace, EndTrace, ECC_Pawn, QueryParams))
	{
		...
		{
			UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), ImpactParticles, Hit.ImpactPoint, Hit.ImpactNormal.Rotation());
	...
	if (MuzzleParticles)
	{
		...
		//UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), MuzzleParticles, Owner->FP_Gun->GetSocketTransform(FName("Muzzle")));
	}
